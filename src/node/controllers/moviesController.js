...
function update(req, res){
    ...
    if(movie !== undefined){
        ...
        movie.grade = newMovieData.grade,
        movie.posterPath = req.file ? '/' + ('uploads/' + req.file.filename) : movie.posterPath
        res.json(movie);
    } else {
        res.status(404).json(`Ce film n'existe pas`);
    }
...
function store(req, res){
    ...
    const newMovie = {
        ...
        grade: newMovieData.grade,
        posterPath: req.file ? '/' + ('uploads/' + req.file.filename) : undefined
    }
